...
api.prototype.ops = function () {
	...
	self.whitelist_restore(function (err) {
		...
		if ((!self.whitelist) || (self.whitelist.length == 0)) {
			var txt = path.resolve(__dirname, "..", "assets/data/whitelist.txt");
			if (fs.existsSync(txt))
				self.whitelist = fs.readFileSync(txt).toString().toLowerCase().split('\n');
		}
		// build cache
...
// ✓ clean database fuckups
api.prototype.db_clean = function (fn) {
	var self = this;
		...
		async.forEachSeries(ents, function (ent, next) {
			...
			ent.data.forEach(function (d) {
				if (d.format == 'string') {
					...
					console.log('TODO:', d.format, d.value);
				}
			});
			...
			if (hasChanged) {
				self.ent_store(ent, function (err) {
...
api.prototype.whitelist_add = function (site, fn) {
	var self = this;
	if (self.whitelist.indexOf(site) >= 0) return fn(new Error("whitelist entry already exists"));
...
api.prototype.whitelist_update = function (site, newsite, fn) {
	var self = this;
	var i = self.whitelist.indexOf(site);
	if (i < 0) return fn(new Error("whitelist entry does not exists"));
	self.whitelist[i] = newsite;
	self.whitelist_save(fn);
};

api.prototype.whitelist_remove = function (site, fn) {
	var self = this;
	if (self.whitelist.indexOf(site) < 0) return fn(new Error("whitelist entry does not exists"));
...
api.prototype.whitelist_save = function (fn) {
	...
	// polyfill callback
	if (typeof fn !== "function") fn = function (err) {
		if (err) debug("whitelist save error: %s", err);
	};
	fs.writeFile(self.opt.whitelist_file, JSON.stringify(
		{error: null, result: self.whitelist}
		, null, '\t'), function (err) {
		if (err) return fn(err);
...
api.prototype.whitelist_restore = function (fn) {
	...
	// polyfill callback
	if (typeof fn !== "function") fn = function (err) {
		if (err) debug("whitelist restore error: %s", err);
	};
	fs.exists(self.opt.whitelist_file, function (exists) {
		if (!exists) return fn(null);
...
api.prototype.update_ent_create_internal = function (entity, fn) {
	...
	self.ent_create(entity, function (err, id) {
		if (err) return fn(err);
		entity.data = [];
		self.db.collection("update-entities").save(entity, function (err) {
			if (err) return fn(err);
			fn(null, id);
		});
	});
...
api.prototype.update_rel_delete = function (id, fn) {
	...
	self.update_rel(id, function (err, rel) {
		...
		self.update_rel_delete_internal(id, function (err) {
			...
			var result = [];
			async.forEachSeries(rel.entities, function (entid, next) {
				self.update_ent_info(entid, function (err, e_update) {
					if (err) return fn(err);
					if (self.update_entity_update_needed(e_update)) {
...
api.prototype.update_entity_info = function (entity, fn) {
	...
	var fields_equal = function (f1, f2) {
		if ((f1.key !== f2.key) || (f1.auto !== f2.auto) || (f1.desc !== f2.desc) || (f1.format !== f2.format)) {
			//console.log('not equal 1', f1, f2);
			return false;
		}
		if (typeof f1.value !== typeof f2.value) {
			console.log('not equal 2', f1, f2);
			return false;
		}
	...
	var rels_equal = function (rel1, rel2) {
		...
		self.update_ent_rels(entity._id, function (err, relations) {
			...
			async.forEachSeries(relations, function (rel, next) {
				...
				async.forEachSeries(rel.entities.filter(function (entid) {
					...
					rel.matches = [];
					if (result.matches.length + rel.entity.matches.length == 2) {
...
api.prototype.update_ent_info = function (id, fn) {
	var self = this;
	self.update_ent(id, function (err, entity) {
		if (err) return fn(err);
...
api.prototype.update_store = function (intermed, fn) {
	var self = this;
	async.forEachSeries(intermed.entities, function (entity, next) {
		self.db.collection("update-entities").save(entity, function (err, result) {
			...
			debug("update entity created: %s", result._id);
			next();
		});
	}, function () {
		async.forEachSeries(intermed.relations, function (rel, next) {
			...
			if (entities.length !== 2) {
				console.log('invalid update relation', entities.length);
				return next();
			}
