...
api.prototype.ent_creaxtend = function(data, fn){
	...
	self.ent_check(data, function(err, ent){
		if (err) return fn(err);
		self.ent_by_name(ent, function(err, result){
			if (err || !result) {
				// create entity
...
// ✓ list all entities
api.prototype.ent_list = function(fn){
	var self = this;
	self.db.collection("entities").find({}, {name: true, type: true, slug: true}).sort({slug: 1}, function(err, result){
		if (err) return fn(err);
		fn(null, result);
	});
	return this;	
...
// ✓ find entity by name
api.prototype.ent_by_name = function(ent, fn){
	//FIXME: improve this.
	debug("get entity by name %s", ent.name);
	var self = this;
	self.db.collection("entities").find({search: {"$in": self.ent_unify(ent)}}, function(err, result){
