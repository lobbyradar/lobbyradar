...
api.prototype.rel_exists = function (ents, fn) {
	...
	self.db.collection("relations").find({"entities": ents}, function (err, result) {
		...
		self.db.collection("relations").find({"entities": ents.reverse()}, function (err, result) {
			if (err) return fn(err, null);
...
api.prototype.update_ent_info = function (id, fn) {
	...
	self.db.collection("update-entities").findOne({_id: self.db.ObjectId(id)}, function (err, entity) {
		if (err) return fn(err);
		if (!entity) return fn('Invalid Id');
		var result = {
			entity: entity,
			relations: []
		};
		self.db.collection("update-relations").find({"entities": {"$in": [self.db.ObjectId(id)]}}, function (err, relations) {
			if (err) return fn(err);
			async.forEachSeries(relations.map(function (rel) {
				return rel;
			}), function (rel, next) {
				async.forEachSeries(rel.entities.filter(function (entid) {
					return (entid.toString() !== id.toString());
				}), function (entid, nextID) {
					self.db.collection("update-entities").findOne({_id: self.db.ObjectId(entid)}, {name: true, slug: true}, function (err, entity) {
						if (!err && entity) {
							debug("got related entity %s", entity._id);
							rel.entity = entity;
...
api.prototype.update_store = function (intermed, fn) {
	...
	async.forEachSeries(intermed.entities, function (entity, next) {
		...
		async.forEachSeries(intermed.relations, function (rel, next) {
			var entites = intermed.entities.filter(function (e) {
				return ((rel.update_id1 == e.update_id) || (rel.update_id2 == e.update_id));
			});
			if (entites.length !== 2) {
				console.log('invalid update relation', entites.length);
				return next();
			}
			rel.entities = entites.map(function (e) {
				return e._id;
			});
