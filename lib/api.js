...
api.prototype.db_clean = function (fn) {
	...
	var ensureBundestagRelation = function (cb) {
		var bundestag_id = '552ff9ceaf9ee96e1c1df7c9';
		async.forEachSeries(mdbs, function (ent, next) {
			self.rel_exists([ent._id, bundestag_id], function (err, result) {
				...
				if (!result) {
					var rel = {
						...
						type: 'member',
						weight: 0,
						entities: [ent._id, bundestag_id],
						tags: ['mdb'],
						data: []
					};
					self.rel_create(rel, function (err, result) {
						if (err) console.log(err);
						console.log(result);
						next();
					});
	...
	self.db.collection("entities").find(function (err, ents) {
		...
		async.forEachSeries(ents, function (ent, next) {
			...
			ensureBundestagRelation(function () {
				fn(null, list);
			});
		});
	});
...
api.prototype.ent_multitags = function (mode, tag, ids, fn) {
	...
	// get entities
	self.db.collection('entities').update({_id: {"$in": _ids}}, action, {multi: true}, function (err, result) {
		if (err) return fn(err);
		fn(null, true);
	});
};
...
api.prototype.rel_multitags = function (mode, tag, ids, fn) {
	...
	// get relations
	self.db.collection('relations').update({_id: {"$in": _ids}}, action, {multi: true}, function (err, result) {
		if (err) return fn(err);
		fn(null, true);
	});
	return this;
