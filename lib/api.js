			...
			return self.rels(function(err, rels){
				if (filterByConnection) {
					result = result.filter(function(item){
						return (rels.hasOwnProperty(item._id) && (rels[item._id].filter(function(s){return s == cond.hasRelationWith;}).length>0));
					});
				}
...
api.prototype.relgraph = function(fn){
	...
	self.db.collection("relations").find({}, {entities: true}, function(err, result){
		...
		var rels = [];
		result.forEach(function(item){
			rels.push(item.entities.map(function(e){ return e.toString(); }));
		});
		fn(null, rels);
	});
	return this;
...
api.prototype.route = function(from, to, fn){
	...
	self.batgirl.find(from, to, function(err, routes){
		...
		_ents = uniq(_ents).map(function(ent){ return self.db.ObjectId(ent); });
		...
		self.db.collection("entities").find({_id: {"$in": _ents}}, { _id: true, name: true, type: true }, function(err, ents){
			...
			self.db.collection("relations").find({entities: {"$in": _rels}}, { _id: true, entities: true, type: true }, function(err, rels){
				...
				rels.forEach(function(rel){
					...
					rel.entities = rel.entities.map(function(e){ return e.toString(); });
...
api.prototype.whitelist_save = function(fn){
	...
	// polyfill callback
	if (typeof fn !== "function") fn = function(err){ if (err) debug("whitelist save error: %s", err); };
