...
api.prototype.reset = function(safephrase, fn){
	var self = this;
	if (safephrase === "i know what i am doing") {
		self.db.collection("entities").remove({}, function(){
			self.db.collection("relations").remove({}, function(){
...
api.prototype.purge = function(importer, fn){
	var self = this;
	self.db.collection("entities").remove({"importer": importer}, function(){
		self.db.collection("relations").remove({"importer": importer}, function(){
			debug("collections emptied");
			fn();
		});
	});
...
api.prototype.rels_full = function(fn){
	debug("all relations");
	var self = this;
	self.db.collection("relations").find(function(err, result){
		if (err) return fn(err);
		if (!result || result.length === 0) return fn(null, []);
		self.db.collection("entities").find({}, {name: true}, function(err, entities){
			if (err) return fn(err);
			var ents = {};
...
api.prototype.field_list = function (fn) {
	debug("list fields");
	var self = this;
	self.db.collection("fields").find().sort({name: 1},function (err, results) {
