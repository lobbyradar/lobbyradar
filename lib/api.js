...
api.prototype.ent_check = function(data, fn){
	...
	if (data.hasOwnProperty("data") && (data.data instanceof Array)){
		data.data.forEach(function(dataset){
			...
			if (!dataset.hasOwnProperty("desc")) dataset.desc = '';
			if (typeof dataset.desc !== "string") return;
			if (!dataset.hasOwnProperty("format") || (typeof dataset.format !== "string") || dataset.format === "") dataset.format = formats[dataset.key]; // FIXME: proper format check
	...
	self.db.collection("entities").find(q, {name: true, type: true, slug: true}).sort({slug: 1}, function(err, result){
		...
		self.rels(function(err, rels){
			result = result.map(function(item){
				item.connections = rels[item._id].length || 0;
				return item;
			});
			fn(null, result);
		});
