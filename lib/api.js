	...
	self.ent_get(id, function(err, a){
		...
		self.ent_check(data, function(err, b){
			...
			Object.keys(a.data).forEach(function(k){
				if (!(b.data.hasOwnProperty(k))) b.data[k] = a.data[k];
			});
			...
			self.ent_update(id, b, function(err, result){
...
api.prototype.rel_check = function(data, fn){
	...
	// check entities
	if (!data.hasOwnProperties("entities") || !(data.entities instanceof Array)) return fn(new Error("entities must not be empty"));
	data.entities = data.entities.map(function(entity){
		return entity.toString();
	}).filter(function(entity){
		return ((typeof entity === "string") && /^[a-f0-9]{24}$/.test(entity));
	});
...
api.prototype.rel_update = function(id, data, fn){
	...
	self.rel_check(data, function(err, data){
		...
		self.db.collection("relations").update({_id: self.db.ObjectId(id)}, data, {upsert: true}, function(err, result){
			if (err) return fn(err);
			debug("relation updated: %s", result._id);
			fn(null, result._id);
		});
	});
	return this;
...
api.prototype.ent_rels = function(id, fn){
	var self = this;
	self.db.find({"entities": { "$in": self.db.ObjectId(id) }}, function(err, result){
		if (err) return fn(err);
		fn(null, result);
	});
	return this;
