...
function api(opt, db){
	...
	self.whitelist_file = path.resolve(__dirname, "..", (self.opt&&self.opt.whitelist||"assets/data/whitelist.json"));
	self.whitelist_restore(function(err){
		if (err) debug("error restoring whitelist data: %s", err);
...
api.prototype.ent_delete = function(id, fn){
	...
	self.db.collection("entities").remove({_id: self.db.ObjectId(id)}, true, function(err, result){
		...
		fn(err, result);
		// delete indexes
		self.index_delete(id, "entity", function(err){
...
api.prototype.rel_delete = function(id, fn){
	...
	self.db.collection("relations").remove({_id: self.db.ObjectId(id)}, true, function(err, result){
		...
		// delete data index
		self.index_delete(id, "relation", function(err){
			if (err) return debug("error deleting index: %s", err);
...
api.prototype.whitelist_save = function(fn){
	...
	fs.writeFile(self.whitelist_file, JSON.stringify(self.whitelist,null,'\t'), function(err){
		if (err) return fn(err);
...
api.prototype.whitelist_restore = function(fn){
	...
	fs.readFile(self.whitelist_file, function(err, data){
		if (err) return fn(err);
		try {
...
api.prototype.whitelist_watch = function(){
	var self = this;
	fs.watch(self.whitelist_file, {persistent: false, resursive: false}, function(evt, filename){
