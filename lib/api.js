...
api.prototype.field_create = function(field, fn) {
	...
	if (!field.type || (!field.type.length)) return fn("field type needed");
	var self = this;
...
api.prototype.field_update = function(id, field, fn) {
	...
	self.field_get(id, function(err, result){
		...
		self.db.collection("fields").update({_id: self.db.ObjectId(user._id)}, result, {upsert: true}, function(err, result){
			if (err) return fn(err);
